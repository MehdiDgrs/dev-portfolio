...
let SkillSliderContainer: React.FC<Props> =({children,offsetX, className,contentWidth}) => {
...
useAnimationFrame(enabled,   useCallback(()=>{
        ...
        if (elContainer && elContent) {
          innerWidth<1100 ? refScrollX.current +=1.5 :  refScrollX.current +=0.7;
...
return(
    ...
    <div ref={refContainer} id="container" className={`slider-container  overflow-x-hidden whitespace-nowrap  w-full bg-white max-w-full pointer-events-none  ${className}`}>
...
export const SliderItem:React.FC<ItemProps> =({children,width}) => {
   return(
    <div className=" justify-center items-center mx-4 inline-block my-6  " style={{width}}>
        {children}
         </div> 
